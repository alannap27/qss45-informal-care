...
long-term care system \cite{jang2012,oecd2023}, and microsimulation projects European
...
0.0874 to 0.0864---that is, not at all. Figure~\ref{fig:b-leakage} shows both
...
My own role is quickly stated. I chose the research question, the outcome
...
a codebook or an output table. What follows spends its space on the four
episodes where the assistant was wrong, because those are the transferable part.
...
matchers, including a lexical baseline that should not have been competitive.
Inspecting the ground-truth table explained it. The assistant had written the
...
nothing in the output signalled it.
...
corrected it. The direction was plausible and the prose was confident. The
computation, run afterward, showed grouped cross-validation scoring
\emph{higher} than naive splitting, with the gap smaller than the between-fold
standard deviation.
...
fabricated finding that no reviewer could have caught from the text.

\paragraph{Failure: two-tier target leakage, introduced by the assistant.} The
nine care-network features were assistant-written and were not flagged as risky
in the code or its comments. Including them lifted held-out $R^2$ from 0.087 to
0.644.
...
\emph{How it was wrong, tier two:} the kin-composition features are zero exactly
...
and their mean informal share is exactly 0.000. This survived a review pass and
a correlation scan, because no single feature looked unusual on its own. It was
found only by asking of each feature what event makes it zero. The assistant
then described the corrected set as ``safe'' in its own commentary, which is the
specific hazard: fluent, well-commented code carrying an error the fluency
conceals.

The root cause is structural rather than clerical. The outcome is
\emph{constructed from} the helper roster, so every roster-derived feature is
...
\paragraph{Failure: silently cropped figures.} A later revision produced figures
whose titles were cut off in the saved files and whose captions rendered in a
colour matching the background. The code looked correct.

\emph{How it was wrong:} \texttt{shap.summary\_plot} mutates the active
Matplotlib figure and the global \texttt{rcParams} as a side effect, so a title
set before that call falls outside the saved bounding box. The assistant's first
two fixes adjusted padding and layout parameters, treating the symptom; neither
worked, and each was proposed with the same confidence as the last. The fix that
held was structural: render each panel to its own file and composite afterward.
An automated check now sweeps every rendered figure for ink touching the canvas
edge, so the same failure cannot recur unnoticed.

\paragraph{Accepted, and why.} One substantive proposal survived scrutiny: the
missing-indicator strategy over native NaN handling in gradient boosting, on the
argument that native handling contaminates SHAP attribution. I accepted it after
confirming the missingness pattern was non-random, and it stands as a
methodological commitment of the paper.

\paragraph{What this implies.} All four failures share a shape: output that is
internally consistent and externally wrong, produced with uniform confidence
whether right or not. None would have been caught by a test suite, because the
...
captions they carry in the repository. This appendix and the Agentic Analysis
section are excluded from the page limit.

\subsection*{A.1 Description}

\begin{figure}[H]
\centering
\includegraphics[width=0.76\linewidth]{figures/f02_helper_composition.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.76\linewidth]{figures/f03_need_gradient.png}
...
\subsection*{A.2 Model comparison}

\begin{figure}[H]
\centering
\includegraphics[width=0.76\linewidth]{figures/f05_importance_ranks.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.76\linewidth]{figures/f10_neural_net.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.76\linewidth]{figures/f07_weights_and_clustering.png}
...
dropped. Household-grouped cross-validation scores \emph{higher} than naive
splitting, which is the opposite of the expected direction and is reported as
such.}
\label{fig:b-weights}
\end{figure}

\subsection*{A.3 Care networks}

\begin{figure}[H]
\centering
\includegraphics[width=0.76\linewidth]{figures/f08_care_networks.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.76\linewidth]{figures/f09_network_features.png}
...
they add anything once the existing covariates are already in the model. They do
not.}
\label{fig:b-netfeat}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.76\linewidth]{figures/f11_leakage.png}
...
the tier-2 kin-composition features still hold it at 0.553, while the genuinely
exogenous tier-3 shape features leave the base covariates where they were. An
improvement of that size on an outcome this degenerate is a symptom rather than a
result. Panel B is the mechanism for tier 1: the informal share drops below one
...
\begin{figure}[H]
\centering
\includegraphics[width=0.76\linewidth]{figures/f12_ablation_and_shap.png}
...
\subsection*{A.4 The cross-national crosswalk}

\begin{figure}[H]
\centering
\includegraphics[width=0.76\linewidth]{figures/f13_embedding_space.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.76\linewidth]{figures/f14_matching_accuracy.png}
...
bottom four, which is what makes the margin usable as a triage rule rather than a
...
\subsection*{A.5 Interaction structure}

\begin{figure}[H]
\centering
\includegraphics[width=0.76\linewidth]{figures/f16_shap_interactions.png}
...
\subsection*{A.6 Omitted tables}

Sixteen tables are the numeric backing for results stated in prose and are not
reproduced here, running from the sample-construction audit and descriptives
through the coefficient and cross-validation comparisons to the ablation ladder,
the crosswalk with its per-item similarities, and the SHAP interaction pairs.
All are committed as CSV in \texttt{output/tables/}.
